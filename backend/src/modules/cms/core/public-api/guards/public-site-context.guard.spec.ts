...
import { PublicSiteContextGuard } from './public-site-context.guard';
...
describe('PublicSiteContextGuard', () => {
  it('resolves the Site from the Host header and attaches it to the request', async () => {
    const site = fakeSite();
    const resolver = { resolveFromHost: jest.fn().mockResolvedValue(site) } as any;
    const guard = new PublicSiteContextGuard(resolver);
    const request: any = { headers: { host: 'nhg.example.com' }, query: {} };
  ...
  it('passes a dev slug from the query param through to the resolver', async () => {
    ...
    const resolver = { resolveFromHost: jest.fn().mockResolvedValue(site) } as any;
    const guard = new PublicSiteContextGuard(resolver);
    const request: any = { headers: { host: 'localhost:3000' }, query: { site: 'nhg' } };
