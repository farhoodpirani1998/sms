...
import { CreateGalleryItemDto, UpdateGalleryItemDto } from './dto/upsert-gallery-item.dto';
import {
  GalleryListQueryDto,
  ReorderGalleryItemsDto,
  ScheduleGalleryItemDto,
  SiteIdQueryDto,
} from './dto/gallery-item-query.dto';
...
/**
 * `cms/gallery` — CMS-H.1. Same guard/permission composition and
 ...
 * `GET/POST /cms/gallery/:id/revisions...` routes. `findAll` uses
 * `GalleryService.findAllByCategory()` rather than `findAll()` directly
 * so the optional `?category=` filter applies without every other
 * content type's controller needing an equivalent.
 */
...
export class GalleryController {
  ...
  @Post()
  async create(@Body() dto: CreateGalleryItemDto, @CurrentUser() user: AuthenticatedUser) {
    const { siteId, ...data } = dto;
    return this.galleryService.create(siteId, data, user.id);
  }

  @Get()
  async findAll(@Query() query: GalleryListQueryDto) {
    return this.galleryService.findAllByCategory(
      query.siteId,
  ...
  async schedule(
    @Param('id') id: string,
    @Body() dto: ScheduleGalleryItemDto,
    @CurrentUser() user: AuthenticatedUser,
  ) {
  ...
  @Post('reorder')
  async reorder(@Body() dto: ReorderGalleryItemsDto, @CurrentUser() user: AuthenticatedUser) {
