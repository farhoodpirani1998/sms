...
let NewsService = class NewsService extends base_content_service_1.BaseContentService {
    constructor(repository, revisionsService, events, publishingService, orderingService, localeResolverService, siteService) {
        super(repository, cms_entity_type_enum_1.CmsEntityType.NEWS_ARTICLE, revisionsService, events);
    ...
    async assertSlugAvailable(siteId, slug, excludeId) {
        ...
        if (existing && existing.id !== excludeId) {
            throw new common_1.ConflictException(`A news article with slug "${slug}" already exists for this Site`);
        }
    }
    ...
    async findPublishedList(siteId, pagination, requestedLocale) {
    ...
    async findPublishedBySlug(siteId, slug, requestedLocale) {
        ...
        const seoTitle = this.localeResolverService.resolveText(article.metaTitle, locale, defaultLocale) ??
            title ??
            this.resolveSiteSeoText(site.seoDefaults?.title, locale, defaultLocale);
        const seoDescription = this.localeResolverService.resolveText(article.metaDescription, locale, defaultLocale) ??
